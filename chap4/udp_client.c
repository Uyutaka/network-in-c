// Based on
// https://github.com/codeplea/Hands-On-Network-Programming-with-C/blob/master/chap04/udp_client.c
// Explaination
...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    // ./upd_client example.com http
    fprintf(stderr, "usage: upd_client hostname port\n");
    return 1;
  }
  ...
  while (1) {
    ...
    // enable cat my_file.txt | upd_client 192.168.54.122 8080.
    if (FD_ISSET(0, &reads)) {
      char read[4096];
